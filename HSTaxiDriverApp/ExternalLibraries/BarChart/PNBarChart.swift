...
class PNBarChart: UIView {
    ...
    var labelMarginTop: CGFloat = 5
    var barBackgroundColor = PNLightGrey
    var labelTextColor = UIColor.darkGray
    var labelTextFont = UIFont(name: APP_FONT_REGULAR, size: 11.0)
    var xLabelSkip = 1
    var yLabelSum = 4
    ...
    func strokeChart() {
        ...
        for value in self.yValues {
            ...
            if self.barWidth != nil && self.barWidth > 0 {
                let firstVal = CGFloat(index) * self.xLabelWidth + self.yChartLabelWidth
                let secondVal = self.chartMargin + (self.xLabelWidth / 2) - (self.barWidth / 2)
                barXPosition = firstVal + secondVal
            } else {
...
//        let max = yLabels.max()
        let max = Utility.shared.rangeMax(max: yLabels.max()!)
        if max == 0 {
