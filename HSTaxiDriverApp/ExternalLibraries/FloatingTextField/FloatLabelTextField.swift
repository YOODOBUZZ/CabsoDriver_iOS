...
let INFO_BUTTON_SIDE_LENGTH_FACTOR = 2
...
@IBDesignable class FloatLabelTextField: UITextField {
    ...
    var titleFont:UIFont = UIFont.init(name: APP_FONT_REGULAR, size: 12)! {
		didSet {
	...
	fileprivate func setup() {
		...
		titleActiveTextColour = PRIMARY_COLOR
		// Set up title label
		title.alpha = 0.0
		title.font = titleFont
		title.textColor = titleTextColour
    ...
    func setAsInvalidTF(_ message: String, in view: UIView) {
        ...
        var infoButton = UIButton(frame: CGRect(x: frame.size.width - (buttonSide + 2.0), y: 2.0, width: buttonSide + 2.0, height: buttonSide))
