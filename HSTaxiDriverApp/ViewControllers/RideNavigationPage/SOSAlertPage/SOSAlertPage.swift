...
class SOSAlertPage: UIViewController,MFMessageComposeViewControllerDelegate {
    ...
    @IBOutlet var messageAlertImageView: UIImageView!
    ...
    @IBAction func alertEmgContactBtnTapped(_ sender: Any) {
        if (UserModel.shared.getEmergenctContact() != nil) {
            if (MFMessageComposeViewController.canSendText()) {
                ...
                controller.body = "Emergency: My Location: https://maps.googleapis.com/maps/api/staticmap?center=\(myLocation.latitude),\(myLocation.longitude)&zoom=16&key=\(GOOGLE_API_KEY)&size=400x200&sensor=false&maptype=roadmap&markers=color:red%7Clabel:S%7C\(myLocation.latitude),\(myLocation.longitude)"
