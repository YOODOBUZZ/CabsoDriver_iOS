    ...
    func resetPasswordService()  {
        ...
        userObj.resetPassword(email: self.emailTF.text!, onSuccess: {response in
            ...
            if status.isEqual(to: STATUS_TRUE){
                AJAlertController.initialization().showAlertWithOkButton(aStrMessage: (response.value(forKey: "message") as? String ?? response.value(forKey: "message") as! String), status: "", completion: { (index, title) in
                    self.dismiss(animated: true, completion: nil)
                })
               
                
            }else if status.isEqual(to: STATUS_FALSE){
                Utility.shared.showAlert(msg:(response.value(forKey: "message") as? String ?? response.value(forKey: "message") as! String), status: "")
            }
        })
    }
