...
//        NotificationCenter.default.addObserver(self, selector: #selector(self.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    ...
    @objc func keyboardWillHide(sender: NSNotification) {
        ...
        if viewType == "password" {
            self.containerView.frame.size.height += keyboardFrame.height+15
        }else if viewType == "name"{
            self.nameEditView.frame.size.height += keyboardFrame.height+15
        }
    }
...
//       }
    ...
    func setupInitialDetails()  {
        ...
        if viewType == "password" {
            ...
            self.nameEditView.frame.size.height = 0
...
//                    Utility.shared.showAlert(msg: Utility.shared.getLanguage()?.value(forKey: "server_alert") as! String)
    ...
    func isValidationSuccess() -> Bool {
        ...
        }else if (self.oldPasswordTF.text?.count)!<6{
            self.oldBorderLbl.backgroundColor = RED_COLOR
            self.oldPasswordTF.setAsInvalidTF("password_limit", in: self.view)
...
//            self.view.frame.size.height = FULL_HEIGHT
//        }
        
//        if viewType == "password" {
//            self.containerView.frame.origin.y = 216
//        }else if viewType == "name"{
//            self.nameEditView.frame.origin.y = FULL_HEIGHT - self.nameEditView.frame.size.height
//        }
    }
}
